...
List<Color> colors=[Colors.yellow,Colors.yellow,Colors.yellow,
                    Colors.green,Colors.green,Colors.green,Colors.green,Colors.green,Colors.green,Colors.green,
                    Colors.green,Colors.green,Colors.green,Colors.green,Colors.green,Colors.green,Colors.red,Colors.red,Colors.red,];
...
class _GraficaBarScreenState extends State<GraficaBarScreen> {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        const Text("DATOS GRAFICA BARRAS",
        ...
        for(Widget i in _datosGrafica2(widget.datos.numeros,colors))
          i
      ],
    );
  }
  List<Widget> _datosGrafica2(List<dynamic> list, List<Color> col){
    ...
    for(int i=0;i<list.length;i++){
      widgets.add(
         figuras(color:col[i],cantidad: i+13,),
      );
      widgets.add(
        SizedBox(
          width: 75,
          child: Row(
            children: [
              Text("${list[i]}",style: const TextStyle(fontSize: 25, fontWeight: FontWeight.bold), )
            ],
          ),
        )
      );
      
      widgets.add(const SizedBox(width: 20, height: 50,));
      if ((i+1)%5==0 )
      {
        widgets.removeLast();
        widgets2.add(
  ...
  Row _datosGrafica(List<String> list) {
    ...
    for(String texto in list){
      widgets.add(
        const figuras(color: Color.fromARGB(255, 240, 208, 2),),
      );
      widgets.add(
        SizedBox(
          width: 60,
          child: Row(
            children: [
              Text(texto,style: const TextStyle(fontSize: 25, fontWeight: FontWeight.bold), )
            ],
          ),
        )
